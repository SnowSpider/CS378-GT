...
void Missile::createObject(Ogre::SceneManager* scnMgr){
    ...
    string s2 = "Smoke_";
    string name2 = s2.append(intToStr(unitId));
    ...
    rootNode->scale(25, 25, 50);
    rootNode->attachObject(scnMgr->createParticleSystem(name2, "Examples/Smoke"));
...
void Missile::relocate(btVector3& dest){
    ...
    btVector3 myDir = start.normalize();
    rootNode->setDirection(Ogre::Vector3(myDir.x(), myDir.y(), myDir.z()), Node::TS_PARENT);
    rootNode->translate(0,0,-300, Node::TS_LOCAL);
    /*
    Ogre::Vector3 myPos = rootNode->getPosition();
    btPos = btVector3(myPos.x, myPos.y, myPos.z);
    rotAxis = btPos.cross(end);
    */
    btVector3 myDirection = rotAxis.cross(start);
    myDirection.normalize(); //critical
    rootNode->setDirection(Ogre::Vector3(myDirection.x(), myDirection.y(), myDirection.z()), Node::TS_PARENT);
}

void Missile::updateAltitude(){
    float x = progress * 6371;
    float k = speed * 0.70710678118;
    altitude = x - ( (9.8*x*x)/(2*k*k) );
}

void Missile::fly(Ogre::SceneManager* scnMgr){
    if(progress < linearDistance){
        rootNode->translate(0,0,-50, Node::TS_LOCAL);
        progress += 50;
        Ogre::Vector3 myPos = rootNode->getPosition();
        btPos = btVector3 (myPos.x, myPos.y, myPos.z);
        rotAxis = btPos.cross(end);
        btVector3 myDirection = rotAxis.cross(btPos);
        
        myDirection.normalize(); //critical
        rootNode->setDirection(Ogre::Vector3(myDirection.x(), myDirection.y(), myDirection.z()), Node::TS_PARENT);
        cout << "linearDistance = " << linearDistance << ", progress = " << progress << endl;
    }
    else{
        cout << "HIT" << endl;
        myEntity->setVisible(false);
        string s2 = "Smoke_";
        string name2 = s2.append(intToStr(id));
        scnMgr->destroyParticleSystem(name2);
        
    }
}

void Bomber::createObject(Ogre::SceneManager* scnMgr){
    string s = "Unit_";
    id = unitId;
    name = s.append(intToStr(unitId));
    unitId += 1;
...
void Bomber::fly(Ogre::SceneManager* scnMgr){
    if(progress < linearDistance){
        //rootNode->setDirection(Ogre::Vector3(0,0,1), Node::TS_LOCAL);
        rootNode->translate(0,0,-50, Node::TS_LOCAL);
        progress += 50;
        Ogre::Vector3 myPos = rootNode->getPosition();
        btPos = btVector3 (myPos.x, myPos.y, myPos.z);
