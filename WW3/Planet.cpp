...
void Planet::createBorderSegments(Ogre::SceneManager* scnMgr, float k){
    ...
    for (int i=0; i<cells.size(); i++){
        ...
        for(int j=0;j<p.size();j+=2){
            ...
            if(cells[i].owner==Owner_RED) manual->begin("MyMaterials/Red", RenderOperation::OT_TRIANGLE_FAN);
            else if(cells[i].owner==Owner_BLUE) manual->begin("MyMaterials/Cyan", RenderOperation::OT_TRIANGLE_FAN);
            else manual->begin("MyMaterials/White", RenderOperation::OT_TRIANGLE_FAN);
                    ...
                    for(int k=0;k<p.size();k+=2){ //critical
                        for(int l=0;l<cells[n[j]].paramVerts.size();l++){
                            if((btVector3)vertices[p[k]] == (btVector3)vertices[cells[n[j]].paramVerts[l]]){
                                ...
                                scnMgr->getEntity(name)->setMaterialName("MyMaterials/Cyan");
                            }
                        }
                    }
                }
                else if(cells[i].owner == Owner_RED){
                    for(int k=0;k<p.size();k+=2){ //critical
                        for(int l=0;l<cells[n[j]].paramVerts.size();l++){
                            if((btVector3)vertices[p[k]] == (btVector3)vertices[cells[n[j]].paramVerts[l]]){
                                int id = cells[i].borderSegments[k/2];
                                ...
                                name = s.append(ss.str()); 
                                scnMgr->getEntity(name)->setVisible(true);
                                scnMgr->getEntity(name)->setMaterialName("MyMaterials/Red");
                            }
                        }
                    }
                }
                ...
                for(int k=0;k<p.size();k+=2){ //critical
                    for(int l=0;l<cells[n[j]].paramVerts.size();l++){
                        if((btVector3)vertices[p[k]] == (btVector3)vertices[cells[n[j]].paramVerts[l]]){
                            ...
                            ss << id;
                            name = s.append(ss.str()); 
                            scnMgr->getEntity(name)->setVisible(false);
                        }
                    }
                }
            }
