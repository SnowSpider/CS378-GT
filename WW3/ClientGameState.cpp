...
void ClientGameState::createScene()
{
    ...
    resultWindow->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::FriendlyReminderButton,this));
    sheet->addChildWindow(resultWindow);
    ...
    w->setVisible( true );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::CommandBaseButton,this));
    w->disable();
    ...
    w->setVisible( true );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::ArmyBaseButton,this));
    w->disable();
    ...
    w->setVisible( true );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::NavyBaseButton,this));
    w->disable();
    ...
    w->setVisible( true );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::AirForceBaseButton,this));
    w->disable();
    ...
    w->setVisible( true );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::NuclearPlantButton,this));
    w->disable();
    ...
    w->setVisible( true );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::ICBMSiloButton,this));
    w->disable();
    ...
    w->setYPosition(CEGUI::UDim(0.05f, 0));
    w->setVisible( false );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::InfantryButton,this));
    w->disable();
    ...
    w->setYPosition(CEGUI::UDim(0.05f, 0));
    w->setVisible( false );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::SubmarineButton,this));
    w->disable();
    ...
    w->setYPosition(CEGUI::UDim(0.20f, 0));
    w->setVisible( false );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::DestroyerButton,this));
    w->disable();
    ...
    w->setYPosition(CEGUI::UDim(0.05f, 0));
    w->setVisible( false );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::TankButton,this));
    w->disable();
    ...
    w->setYPosition(CEGUI::UDim(0.20f, 0));
    w->setVisible( false );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::ScudButton,this));
    w->disable();
    ...
    w->setYPosition(CEGUI::UDim(0.05f, 0));
    w->setVisible( false );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::BomberButton,this));
    w->disable();
    ...
    w->setYPosition(CEGUI::UDim(0.20f, 0));
    w->setVisible( false );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::FighterButton,this));
    w->disable();
    ...
    w->setYPosition(CEGUI::UDim(0.05f, 0));
    w->setVisible( false );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::CaptureButton,this));
    w->disable();
    ...
    w->setYPosition(CEGUI::UDim(0.05f, 0));
    w->setVisible( false );
    w->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::LaunchButton,this));
    w->disable();
    ...
    sounder->subscribeEvent(CEGUI::PushButton::EventClicked,CEGUI::Event::Subscriber(&ClientGameState::soundButton,this));
...
{
    CEGUI::System::getSingleton().injectMouseButtonDown(ClientGameState::convertButton(id));
    if(onButton)
    {
        onButton = false;
        return true;
...
void ClientGameState::onRightPressed(const OIS::MouseEvent &evt){
    deselect();
}
...
CEGUI::MouseButton ClientGameState::convertButton(OIS::MouseButtonID buttonID)
{
    switch (buttonID)
    {
    case OIS::MB_Left:
...
bool ClientGameState::soundButton(const CEGUI::EventArgs &e)
{
    ClientGameState::soundSwitch();
    onButton = true;
    return true;
...
bool ClientGameState::soundSwitch(void)
{
...
bool ClientGameState::CommandBaseButton(const CEGUI::EventArgs &e)
{
...
bool ClientGameState::ArmyBaseButton(const CEGUI::EventArgs &e)
{
...
bool ClientGameState::NavyBaseButton(const CEGUI::EventArgs &e)
{
...
bool ClientGameState::AirForceBaseButton(const CEGUI::EventArgs &e)
{
...
bool ClientGameState::NuclearPlantButton(const CEGUI::EventArgs &e)
{
...
bool ClientGameState::ICBMSiloButton(const CEGUI::EventArgs &e)
{
...
bool ClientGameState::InfantryButton(const CEGUI::EventArgs &e)
{
    for(int i=0;i<currentCell->neighbors.size();i++){
        PlanetCell& temp = earth.cells[currentCell->neighbors[i]];
        if(temp.terrain != Terrain_WATER && temp.myUnit == Unit_EMPTY)
...
bool ClientGameState::TankButton(const CEGUI::EventArgs &e)
{
    for(int i=0;i<currentCell->neighbors.size();i++){
        PlanetCell& temp = earth.cells[currentCell->neighbors[i]];
        if(temp.terrain != Terrain_WATER && temp.myUnit == Unit_EMPTY)
...
bool ClientGameState::ScudButton(const CEGUI::EventArgs &e)
{
    for(int i=0;i<currentCell->neighbors.size();i++){
        PlanetCell& temp = earth.cells[currentCell->neighbors[i]];
        if(temp.terrain != Terrain_WATER && temp.myUnit == Unit_EMPTY)
...
bool ClientGameState::SubmarineButton(const CEGUI::EventArgs &e)
{
    for(int i=0;i<currentCell->neighbors.size();i++){
        PlanetCell& temp = earth.cells[currentCell->neighbors[i]];
        if(temp.terrain != Terrain_LAND && temp.myUnit == Unit_EMPTY)
...
bool ClientGameState::DestroyerButton(const CEGUI::EventArgs &e)
{
    for(int i=0;i<currentCell->neighbors.size();i++){
        PlanetCell& temp = earth.cells[currentCell->neighbors[i]];
        if(temp.terrain != Terrain_LAND && temp.myUnit == Unit_EMPTY)
...
bool ClientGameState::BomberButton(const CEGUI::EventArgs &e)
{
    for(int i=0;i<currentCell->neighbors.size();i++){
        PlanetCell& temp = earth.cells[currentCell->neighbors[i]];
        if(temp.myUnit == Unit_EMPTY)
...
bool ClientGameState::FighterButton(const CEGUI::EventArgs &e)
{
    for(int i=0;i<currentCell->neighbors.size();i++){
        PlanetCell& temp = earth.cells[currentCell->neighbors[i]];
        if(temp.myUnit == Unit_EMPTY)
...
bool ClientGameState::CaptureButton(const CEGUI::EventArgs &e)
{
...
bool ClientGameState::LaunchButton(const CEGUI::EventArgs &e)
{
    return true;
}

bool ClientGameState::FriendlyReminderButton(const CEGUI::EventArgs &e)
{
    if(tutorial == 0){
...
void ClientGameState::BuildingImages1(PlanetCell &cell)
{
    CEGUI::Window* w;
    if(buildingImages != 1)
    {
...
void ClientGameState::BuildingImagesCB2(PlanetCell &cell)
{
    CEGUI::Window* w;
    if(buildingImages != 2)
    {
...
void ClientGameState::BuildingImagesA3(PlanetCell &cell)
{
    CEGUI::Window* w;
    if(buildingImages != 3)
    {
...
void ClientGameState::BuildingImagesN4(PlanetCell &cell)
{
    CEGUI::Window* w;
    if(buildingImages != 4)
    {
...
void ClientGameState::BuildingImagesAF5(PlanetCell &cell)
{
    CEGUI::Window* w;
    if(buildingImages != 5)
    {
...
void ClientGameState::BuildingImagesNP6(PlanetCell &cell)
{
    CEGUI::Window* w;
    if(buildingImages != 6)
    {
...
void ClientGameState::UnitImagesI1(PlanetCell &cell)
{
    CEGUI::Window* w;
    if(buildingImages != 7)
    {
...
void ClientGameState::UnitImagesT2(PlanetCell &cell)
{
    CEGUI::Window* w;
    if(buildingImages != 8)
    {
...
void ClientGameState::UnitImagesS3(PlanetCell &cell)
{
    CEGUI::Window* w;
    if(buildingImages != 9)
    {
...
void ClientGameState::UnitImagesSM4(PlanetCell &cell)
{
    CEGUI::Window* w;
...
void ClientGameState::UnitImagesD5(PlanetCell &cell)
{
    CEGUI::Window* w;
...
void ClientGameState::UnitImagesB6(PlanetCell &cell)
{
    CEGUI::Window* w;
...
void ClientGameState::UnitImagesF7(PlanetCell &cell)
{
    CEGUI::Window* w;
...
void ClientGameState::BuildButtons(void)
{
    if(currentCell == NULL)
        BuildingImages0();
...
bool ClientGameState::issueProduceOrder(PlanetCell& origin, PlanetCell& goal){
    for(int i=0;i<lastCell->neighbors.size();i++){
...
bool ClientGameState::createUnit(PlanetCell& goal){
    if(goal.myUnit == 0)
    {
...
bool ClientGameState::issueMoveOrder(Unit& unit, PlanetCell& origin, PlanetCell& goal){
...
bool ClientGameState::moveUnit(Unit& unit, PlanetCell& origin, PlanetCell& goal){
    origin.moving = false;
    origin.goalId = -1;
...
void ClientGameState::illuminate(PlanetCell& goal){
...
void ClientGameState::deluminate(PlanetCell& goal){
...
float ClientGameState::y(float x, float v, float theta){
    float k = v * cos(theta);
    return x * tan(theta) - ( (9.8*x*x)/(2*k*k) );
}

void ClientGameState::fireMissile(PlanetCell& origin, PlanetCell& goal){
...
void ClientGameState::killEnemyUnit(PlanetCell& targetCell){
    if(targetCell.myUnit == Unit_COMMANDBASE){
...
void ClientGameState::nuke(PlanetCell& targetCell){
    units[targetCell.myUnitId].destroy(m_pSceneMgr);
    deluminate(targetCell);
    targetCell.myUnit = 0;
    targetCell.myUnitId = -1;
...
void ClientGameState::processEvents(void){
...
void ClientGameState::DeButton(void)
{
    CEGUI::Window* w;
    if(buildingImages == 1)
    {
...
void ClientGameState::processResources(int inputmoney, int inputplut)
{
    money += inputmoney;
    plutonium += inputplut;
...
void ClientGameState::processTimer(void)
{
    std::time(&currentTime);
...
void ClientGameState::capture(PlanetCell& targetCell)
{
...
void ClientGameState::attack(PlanetCell& targetCell)
{
...
void ClientGameState::combat(PlanetCell& yourCell,PlanetCell& enemyCell)
{
...
void ClientGameState::initNetwork(void)
{
...
void ClientGameState::updateSend(PlanetCell &cell1,PlanetCell &cell2,PlanetCell &cell3,PlanetCell &cell4,PlanetCell &cell5,PlanetCell &cell6,PlanetCell &cell7)
{
